...
class Form(wx.Panel):
    ...
    def onSave(self,event):
        self.__log('User clicked on button with id %s'%event.GetEventObject())

    def onNameEntered(self, event):
        self.__log('User entered name: %s'%event.GetEventObject())

    def onNameChanged(self, event):
        self.__log('User typed character: %d'%event.GetId())
        event.Skip()

    def onInsuranceChanged(self, event):
        self.__log('User wants insurance: %s'%event.GetId())
        self.__log('User wants insurance: %s'%event.GetEventObject().GetId())
